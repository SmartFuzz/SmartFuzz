...
static void __afl_start_forkserver(void) {
  // 这段代码片段背后的基本思路是向 afl-fuzz 告知目标二进制文件的相关信息。我们要告知它映射大小、是否有字典，以及是否使用共享内存模糊测试。然后我们将结果写入 FORKSRV_FD+1，正如我们之前所讨论的，这是状态管道。
  u8 tmp[4] = {0, 0, 0, 0};
  u32 status = 0;
...
static u32 __afl_next_testcase(u8 *buf, u32 max_len) {
  ...
  if (read(FORKSRV_FD, &status, 4) != 4) {
    // std::cerr << "read from forkserver failed, errno: " << errno << ", error message: " << strerror(errno) << std::endl;
    return 0;
  }

  /* we have a testcase - read it */
  status = read(0, buf, max_len);
  // std::cerr << "have a testcase with size " << status << std::endl;
  /* report that we are starting the target */
  if (write(FORKSRV_FD + 1, &res, 4) != 4) {
    // std::cerr << "write to forkserver failed" << std::endl;
    return 0;
  }
...
int main(int argc, char *argv[]) {
  std::cout << GREEN << "[" << "INFO" << "] "
          << "start db driver" << RESET << std::endl;
  ...
  if (!database->check_alive()) {
     std::cout << GREEN << "[" << "INFO" << "] "
            << "MonetDBClient::check_alive false,starting server" << RESET << std::endl;
    system(startup_cmd.c_str());
    sleep(5);
  }
  std::cerr << "check_alive: success"  << std::endl;
