...
#define RIGHTCOMMENTINSERT \
  if (right_comment_ != NULL){\
    res->right_comment = right_comment_->to_string();\
  }

#define LEFTCOMMENTINSERT \
  if (left_comment_ != NULL) {\
    res->left_comment = left_comment_->to_string();\
  }
#define COMMENTINSERT \
  LEFTCOMMENTINSERT \
  RIGHTCOMMENTINSERT
#define TRANSLATEEND             \
  ...
  COMMENTINSERT                   \
                                 \
  return res;
// TODO ：确认TRANSLATEENDNOPUSH是否需要添加 comment 添加
