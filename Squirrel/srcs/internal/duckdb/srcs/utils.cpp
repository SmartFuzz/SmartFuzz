...
Program *parser_with_error_recover(string sql, int cnt,bool is_recover,bool is_print) {
  ...
  if (hsql_lex_init(&scanner)) {  // 初始化词法分析器
    return NULL;                // 初始化失败返回NULL
  }
  state =
      hsql__scan_string(sql.c_str(), scanner);  // 将SQL字符串绑定到扫描器缓冲区
  int ret = hsql_parse(p, scanner);  // 调用解析器处理SQL，结果存入Program对象

  hsql__delete_buffer(state, scanner);  // 释放缓冲区
  hsql_lex_destroy(scanner);            // 销毁词法分析器
  if (ret != 0 && is_recover) {
