...
IR *Mutator::deep_copy_with_record(const IR *root, const IR *record) {
  ...
  // 复制 comment 的内容
  copy_res->left_comment = root->left_comment;
  copy_res->right_comment = root->right_comment;
...
vector<IR *> Mutator::mutate_all(vector<IR *> &v_ir_collector) {
  ...
  for (auto ir : v_ir_collector) {
    ...
    for (auto i : v_mutated_ir) {
      ...
      if (!check_node_num(new_ir_tree, 150)) {
...
void Mutator::init(string f_testcase, string f_common_string, string pragma) {
  ...
  while (getline(input_test, line)) {
    auto p = parser_with_error_recover(line);
    if (p == NULL) {
      cout << "[error] init parse sql error:" << line << endl;
      continue;
    }
    ...
    deep_delete(res);
    p = parser_with_error_recover(strip_sql);

    if (p == NULL) {
      cout << "[error] init parse strip_sql error:" << strip_sql << endl;
      continue;
    }
...
string Mutator::validate(IR *root) {
  ...
  try {
    ...
    auto parsed_ir = parser_with_error_recover(sql_str);
...
/**
 * @brief 构建 IR 节点的依赖关系图，用于后续修复操作
 * 
 * @param root IR 树的根节点指针
 * @param relationmap 存储 IDTYPE 之间的映射关系，用于查找匹配的 IR 节点
 * @param cross_map 存储跨语句的 IDTYPE 映射关系
 * @param ordered_ir 用于存储需要修复的 IR 节点列表
 * @return map<IR *, set<IR *>> 依赖关系图，键为 IR 节点指针，值为该节点依赖的 IR 节点集合
 */
map<IR *, set<IR *>> Mutator::build_dependency_graph(
    IR *root, map<IDTYPE, IDTYPE> &relationmap, map<IDTYPE, IDTYPE> &cross_map,
    vector<IR *> &ordered_ir) {
  // 初始化依赖关系图，键为 IR 节点指针，值为该节点依赖的 IR 节点集合
  map<IR *, set<IR *>> graph;
  // 存储需要修复的 IDTYPE 集合
  set<IDTYPE> type_to_fix;
  // 用于在切割子查询时保存被移除的子查询节点，键为指向 IR 指针的指针，值为 IR 节点指针
  map<IR **, IR *> m_save;
  // 遍历 relationmap，将其中的 IDTYPE 插入到 type_to_fix 集合中
  for (auto &iter : relationmap) {
    type_to_fix.insert(iter.first);
    type_to_fix.insert(iter.second);
  }
  // 切割子查询:
  // 调用 cut_subquery 方法将 root 中的子查询切割出来，返回一个包含所有语句的 IR 节点列表 ir_list，
  // 同时将被移除的子查询节点保存到 m_save 中。
  auto ir_list = cut_subquery(root, m_save);
  // 遍历切割后的语句
  for (auto stmt : ir_list) {
    // 存储当前语句中需要修复的 IR 节点
    vector<IR *> ir_to_fix;
    // 调用 collect_ir 方法收集当前语句中需要修复的 IR 节点到 ir_to_fix 向量中
    collect_ir(stmt, type_to_fix, ir_to_fix);
    // 将当前语句中需要修复的 IR 节点添加到 ordered_ir 列表中
    for (auto ii : ir_to_fix) {
      ordered_ir.push_back(ii);
    }
    // 调用 cross_stmt_map 方法根据 cross_map 构建跨语句的依赖关系
    cross_stmt_map(graph, ir_to_fix, cross_map);
    // 存储顶级表名的 IR 节点
    vector<IR *> v_top_table;
    // 调用 toptable_map 方法处理顶级表名的依赖关系，结果存储在 v_top_table 中
    toptable_map(graph, ir_to_fix, v_top_table);
    // 构建依赖关系图
    for (auto ir : ir_to_fix) {
      // 获取当前 IR 节点的 IDTYPE
      auto idtype = ir->id_type_;
      // 清空当前节点在图中的依赖节点集合（这里调用 empty() 无实际效果，可能是代码失误，应使用 clear()）
      graph[ir].empty();
      // 若 relationmap 中不存在当前节点的 IDTYPE，则跳过该节点
      ...
      while (true) {
        ...
        // 调用 search_mapped_ir 方法在兄弟节点及其子树中查找与 relationmap[idtype] 匹配的 IR 节点
        ...
        if (match_ir != NULL) {
          ...
          if (ir->type_ == kColumnName && ir->left_ != NULL) {
            ...
            // 将当前节点的左子节点添加到匹配节点的依赖节点集合中
            graph[match_ir].insert(ir->left_);
            // 若当前节点的右子节点不为空，将其添加到匹配节点的依赖节点集合中
            if (ir->right_) {
              ...
              // 设置当前节点的 IDTYPE 为任意类型
              ir->id_type_ = id_whatever;
            }
          } else
            // 将当前节点添加到匹配节点的依赖节点集合中
  ...
  // 恢复子查询:
  // 调用 fix_back 方法将之前切割子查询时保存的子查询节点恢复到原来的位置
  fix_back(m_save);
  // 返回构建好的依赖关系图
  return graph;
}
...
/**
 * @brief 依据依赖关系图修复 IR 节点，确保 IR 节点的表名、列名等属性符合规则
 * 
 * @param graph 依赖关系图，键为 IR 节点指针，值为该节点依赖的 IR 节点集合
 * @param root IR 树的根节点指针
 * @param ordered_ir 需要修复的 IR 节点列表
 */
void Mutator::fix_graph(map<IR *, set<IR *>> &graph, IR *root,
                        vector<IR *> &ordered_ir) {
  // 用于记录已经访问过的 IR 节点，避免重复处理
  set<IR *> visited;

  // 重置数据库相关信息，清空 m_tables 和 v_table_names
  reset_database();

  // 遍历需要修复的 IR 节点列表
  for (auto ir : ordered_ir) {
    // 创建一个 pair，第一个元素为当前 IR 节点指针，第二个元素为该节点在依赖图中的依赖节点集合
    auto iter = make_pair(ir, graph[ir]);

    // 检查当前节点是否已经访问过，如果是则跳过本次循环
    ...
    if (iter.second.empty()) {
      ...
      if (iter.first->id_type_ == id_column_name) {
        // 从 v_table_names 中随机选取一个表名
        string tablename = vector_rand_ele(v_table_names);
        // 获取该表对应的列名集合
        auto &colums = m_tables[tablename];
        // 从列名集合中随机选取一个列名，赋值给当前节点的字符串值
...
int Mutator::try_fix(char *buf, int len, char *&new_buf, int &new_len) {
  string sql(buf);
  auto ast = parser_with_error_recover(sql);
