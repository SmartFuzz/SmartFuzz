...
DuckDB *create_duckdb() { return new DuckDB; }
DuckDB::DuckDB() { mutator_ = std::make_unique<Mutator>(); }

bool DuckDB::initialize(YAML::Node config) {
...
bool DuckDB::save_interesting_query(const std::string &query) {
  if (Program *program = parser_with_error_recover(query)) {
    ...
    auto p_strip_sql = parser_with_error_recover(strip_sql);
    if (p_strip_sql) {
...
size_t DuckDB::validate_all(const std::vector<IR *> &ir_set) {
  for (IR *ir : ir_set) {
...
bool DuckDB::has_mutated_test_cases() {
  return !validated_test_cases_.empty();
}

size_t DuckDB::mutate(const std::string &query) {
  std::vector<IR *> ir_set, mutated_tree;
  Program *program_root = parser_with_error_recover(query.c_str());
...
std::string DuckDB::get_next_mutated_query() {
