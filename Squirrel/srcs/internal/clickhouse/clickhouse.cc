...
CLICKHOUSE *create_clickhouse() { return new CLICKHOUSE; }

CLICKHOUSE::CLICKHOUSE() { mutator_ = std::make_unique<Mutator>(); }

bool CLICKHOUSE::initialize(YAML::Node config) {
  cout<<GREEN<<"[INFO] initialize clickhouse"<<RESET<<endl;
  ...
  cout<<GREEN<<"[INFO] initialize clickhouse fuzzer success"<<RESET<<endl;
  return true;
}

bool CLICKHOUSE::save_interesting_query(const std::string &query) {
...
size_t CLICKHOUSE::validate_all(std::vector<IR *> &ir_set) {
  for (IR *&ir : ir_set) {
...
bool CLICKHOUSE::has_mutated_test_cases() {
  return !validated_test_cases_.empty();
}

size_t CLICKHOUSE::mutate(const std::string &query) {
...
std::string CLICKHOUSE::get_next_mutated_query() {
