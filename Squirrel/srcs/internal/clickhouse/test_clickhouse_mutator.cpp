...
int main() {
  ...
  // CLICKHOUSE* db = create_clickhouse();
  // const char* config_file_path = "/home/Squirrel/data/config_clickhouse.yml";
  ...
  // // 读取文件中的 sql
  // string sql = get_string_in_file(
  //     "/tmp/fuzz/smart_fuzz/monetdb_20250912_f220b9/queue/.state/"
  //     "variable_behavior/"
  //     "id:000229,time:0,execs:0,orig:function___insert_s1__int_pos__int_nr__s2_"
  //     "__.json.txt");
  // cout << GREEN << "sql:" << sql << RESET << endl;
  // size_t size = db->mutate(sql);
  // cout << GREEN << "size:" << size << RESET << endl;
  // if (size > 0) {
