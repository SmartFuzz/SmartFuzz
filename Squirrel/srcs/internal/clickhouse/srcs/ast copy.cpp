...
IR *Program::translate(vector<IR *> &v_ir_collector) {
  ...
  if (left_comment_ != NULL) {
    auto left_comment_ir = left_comment_->translate(v_ir_collector);
    tmp1 = new IR(kUnknown, OP3("", "", ""), left_comment_ir, tmp1);
  }
  //  new IR(kProgram, OP3("", "", ""), left_comment_ir, tmp1);

  res = new IR(kProgram, OP3("", "", ""), tmp1);
  if (right_comment_ != NULL) {
    auto right_comment_ir = right_comment_->translate(v_ir_collector);
    res = new IR(kUnknown, OP3("", "", ""), res, right_comment_ir);
  }
  TRANSLATEEND
}
