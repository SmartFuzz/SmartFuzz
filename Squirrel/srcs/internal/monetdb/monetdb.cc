...
MONETDB *create_monetdb() { return new MONETDB; }

MONETDB::MONETDB() { mutator_ = std::make_unique<Mutator>(); }

bool MONETDB::initialize(YAML::Node config) {
  cout<<GREEN<<"[INFO] initialize monetdb"<<RESET<<endl;
  const std::string init_lib_path = config["init_lib"].as<std::string>();
  std::string data_lib = config["data_lib"].as<std::string>();
  std::string seed_input = config["seed_input"].as<std::string>();
  std::vector<std::string> file_list =
      get_all_files_in_dir(init_lib_path.c_str());
  if(getenv("SEED_INPUT_INIT")){
    std::vector<std::string> seed_list =
        get_all_files_in_dir(seed_input.c_str());
    for (auto &f : seed_list) {
      // 代表输入了一个绝对路径：/home/xxx/1.txt
      mutator_->init(absl::StrFormat("%s/%s", seed_input, f));
    }
  }
  ...
  cout<<GREEN<<"[INFO] initialize monetdb fuzzer success"<<RESET<<endl;
  return true;
}

bool MONETDB::save_interesting_query(const std::string &query) {
...
size_t MONETDB::validate_all(std::vector<IR *> &ir_set) {
  for (IR *&ir : ir_set) {
...
bool MONETDB::has_mutated_test_cases() {
  return !validated_test_cases_.empty();
}

size_t MONETDB::mutate(const std::string &query) {
  ...
  debug_log(cout << GREEN <<  "[Fatal] validated ir size: " << validated_ir_size << std::endl;);
  return validated_ir_size;
}

std::string MONETDB::get_next_mutated_query() {
