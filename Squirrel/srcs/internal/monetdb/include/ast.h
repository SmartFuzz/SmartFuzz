...
class CommentLiteral;// 解决循环引用的问题
class Node {
 public:
  ...
  Node(){};
  ~Node(){};
  CommentLiteral* left_comment_ = NULL;
  CommentLiteral* right_comment_ = NULL;
};
...
class Program : public Node {
 public:
  ...
  virtual IR* translate(vector<IR*>& v_ir_collector);
  virtual void generate();
  std::vector<ErrorPosition> errors;  // 添加错误位置列表
  Stmtlist* stmtlist_;
};
...
class Literal : public Node {
 public:
  ...
  CommentLiteral* comment_literal_;
};
...
class CommentLiteral : public Node {
 public:
  virtual void deep_delete();
  virtual IR* translate(vector<IR*>& v_ir_collector);
  virtual void generate();

  vector<string> string_vals_;
  string to_string();
};
