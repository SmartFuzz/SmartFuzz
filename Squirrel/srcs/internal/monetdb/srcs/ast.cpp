...
string IR::to_string_core() {
  ...
  switch (type_) {
    case kIntLiteral:
      return left_comment + " " + std::to_string(int_val_) + " " +
             right_comment;
    case kFloatLiteral:
      return left_comment + " " + std::to_string(float_val_) + " " +
             right_comment;
    case kIdentifier:
      return left_comment + " " + str_val_ + " " + right_comment;
    case kStringLiteral:
      return left_comment + " " + str_val_ + " " + right_comment;
      // case kCommentLiteral:
      //   return str_val_;
  }
...
IR *ConstraintType::translate(vector<IR *> &v_ir_collector) {
  ...
  CASEEND
  CASESTART(3)
  auto tmp1 = SAFETRANSLATE(opt_check_);
...
IR *CastExpr::translate(vector<IR *> &v_ir_collector) {
  ...
  auto tmp2 = SAFETRANSLATE(type_name_);
  res = new IR(kCastExpr, OP3("", "", ""), tmp2, tmp1);
  CASEEND
  SWITCHEND
  TRANSLATEEND
}
...
string extractComment(const string& str) {
...
string CommentLiteral::to_string() {
  string comment_str = "";
  for (auto string_val_ : string_vals_) {
    comment_str += " ";
    comment_str += extractComment(string_val_);
    
  }
  // cout << CYAN << "CommentLiteral::comment_str: " << comment_str << RESET << endl;
  return comment_str;
}
IR *CommentLiteral::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART
  // assert(false);
  // left_comment_ vector 转换成 string
  // std::string comment_str = "";
  // for (auto string_val_ : string_vals_) {
  //   comment_str += string_val_;
  // }
  // cout <<CYAN << "CommentLiteral::comment_str: " << comment_str<< RESET << endl;

  // res = new IR(kCommentLiteral, comment_str, data_type_, scope_, data_flag_);

  TRANSLATEEND
}

void CommentLiteral::deep_delete() { delete this; };

void CommentLiteral::generate() {
  // TODO: 研究一下这个函数
  GENERATESTART(1)

  string_vals_.push_back(gen_string());
...
IR *ColumnName::translate(vector<IR *> &v_ir_collector) {
  ...
  CASESTART(1)
  auto tmp1 = SAFETRANSLATE(identifier_);
  IR *tmp2 = SAFETRANSLATE(identifier2_);
...
IR *FunctionName::translate(vector<IR *> &v_ir_collector) {
  ...
  CASEEND
  CASESTART(1) 
   res = new IR(kFunctionName, OP3("sys.", "", ""), tmp1);
  CASEEND
  SWITCHEND
  TRANSLATEEND
}
...
IR *CharacterWithoutLength::translate(vector<IR *> &v_ir_collector) {
  ...
  CASEEND
  CASESTART(4)
  res = new IR(kCharacterWithoutLength, OP3("DATE", "", ""));
  CASEEND
  CASESTART(5)
  res = new IR(kCharacterWithoutLength, OP3("TIME", "", ""));
  CASEEND
  CASESTART(6)
  res = new IR(kCharacterWithoutLength, OP3("TIMESTAMP", "", ""));
  CASEEND
  CASESTART(7)
  res = new IR(kCharacterWithoutLength, OP3("inet", "", ""));
  CASEEND
  CASESTART(8)
  res = new IR(kCharacterWithoutLength, OP3("JSON", "", ""));
  CASEEND
  CASESTART(9)
  res = new IR(kCharacterWithoutLength, OP3("URL", "", ""));
  CASEEND
  CASESTART(10)
  res = new IR(kCharacterWithoutLength, OP3("uuid", "", ""));
  CASEEND
  CASESTART(11)
  res = new IR(kCharacterWithoutLength, OP3("INTERVAL", "", ""));
  CASEEND
  SWITCHEND
...
IR *CharacterConflicta::translate(vector<IR *> &v_ir_collector) {
  ...
  CASEEND
  CASESTART(10)
  res = new IR(kCharacterConflicta, OP3("TIME", "", ""));
  CASEEND
  CASESTART(11)
  res = new IR(kCharacterConflicta, OP3("", "", ""));
  CASEEND
  SWITCHEND
