...
Program *parser_with_error_recover(string sql, int cnt=1,bool is_recover=true,bool is_print=true) {
  ...
  if(is_print  /*&& getenv(DEBUG_MODE.c_str())*/) std::cout << MAGENTA << "[INFO] parsing sql:"<<sql << RESET << std::endl;
  yyscan_t scanner;            // 声明词法分析器实例
  YY_BUFFER_STATE state;       // 声明缓冲区状态对象
  Program *p = new Program();  // 创建Program对象存储解析结果
  ...
  if (ret != 0 && is_recover) {
    ...
    for (const auto &err : p->errors) {
      ...
      if (start< 0 && start >= modified.size()) {
        cout<<"error recovery 异常" <<endl;
        return NULL;
      }
      if (modified[start] == ';' ) {
      ...
      if(1|| getenv("COMMENT_STRING")){
        if (  modified[start] == '\'' ) {
          char string_listeral=modified[start];
            //向前找到最近的'
            int i=start-1;
            while(i>=0 && modified[i]!=string_listeral){
                i--;
            }
            if(i<0){
                return NULL;
