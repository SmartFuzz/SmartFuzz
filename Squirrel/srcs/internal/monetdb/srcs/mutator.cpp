...
//#define GRAPHLOG
/** 从 root 开始深拷贝，并且记录新 record 到 this_record_ */
...
vector<IR *> Mutator::mutate_all(vector<IR *> &v_ir_collector) {
  ...
  for (auto ir : v_ir_collector) {
    if (not_mutatable_types_.find(ir->type_) != not_mutatable_types_.end())
      continue;
    // IR->根据策略变异 变成 <IR1,IR2,IR3>
    vector<IR *> v_mutated_ir = mutate(ir);
    // 变异后生成新的 IR
    for (auto i : v_mutated_ir) {
      IR *new_ir_tree = deep_copy_with_record(root, ir);// 复制原来的 tree 成 new_ir_tree
      replace(new_ir_tree, this->record_, i);//把new_ir_tree 中旧的 IR 替换成新的变异的 IR1

      extract_struct(new_ir_tree);
      string tmp = new_ir_tree->to_string();//
      // cout<<GREEN<<"[INFO] [*]temp mutated SQL: "<<tmp<<RESET<<endl;
...
void Mutator::init_ir_library(string filename) {
  ...
  while (getline(input_file, line)) {
    ...
    count++;
    // auto p = parser(line);
    auto p = parser_with_error_recover(line,1,true,false);

    if (p == NULL) {
      cout << "[error] init_ir_library init sql error:" << line << endl;
      continue;
    }
    vector<IR *> v_ir;
  ...
  // 输出count，success_count
  cout<<GREEN<<"[INFO] initialize "<<filename<<" , total contains "<<count<<" stmts,  "<<success_count<<" stmts init success!"<<RESET<<endl;
...
IR *Mutator::strategy_insert(IR *cur) {
  ...
  if (res->right_ == NULL && res->left_ != NULL) {
    ...
    for (int k = 0; k < 4; k++) { // 这个 4 是不是有点随意了
...
void Mutator::extract_struct(IR *root) {
  ...
  if (root->data_type_ != kDataWhatever) {// identifier,type,functional
    root->str_val_ = "x";
    return;
  }
  // kDataWhatever就是一些常量，正则规约化
...
/* 验证IR转化成 sql query 是否合法 */
bool Mutator::validate(IR *&root) {
  reset_data_library(); // TODO：思考为什么需要 reset data library
  // TOFIX： 需要确认是否需要 comment 实例化
  string sql = root->to_string();
  debug_log(cout << GREEN << "validating sql:\n" << sql << RESET << endl;)
  
  auto ast = parser_with_error_recover(sql,1,true,false);
  if (ast == NULL) {
    
    debug_log(cout << GREEN << "[-] parse fail" << RESET << endl;)
    return false;
  }
 
  debug_log(cout << GREEN << "[-] parse success" << RESET << endl;)
  deep_delete(root);
  root = NULL;
  ...
  if (fix(root) == false) {
    debug_log(cout << GREEN << "[-] fix error" << RESET << endl;)
    return false;
  }
...
bool Mutator::fix(IR *root) {
  ...
  int max_stmts = 8; // 旧值为 8
  if (stmts.size() > max_stmts) {
    connect_back(m_save);
    debug_log(cout << GREEN << "[-] fix error: stmt num > " << max_stmts << RESET << endl;)
    return false;
  }
  ...
  for (auto &stmt : stmts) {
    ...
    if (stmt_num > 4) {
      ...
      debug_log(cout << GREEN << "[-] fix error: stmt num > 4" << RESET << endl;)
    ...
    for (auto &substmt : substmts) {
      ...
      if ((stmt_num == 1 && tmp_node_num > 150) || tmp_node_num > 120) {
        connect_back(m_save);
        connect_back(m_substmt_save);
        debug_log(cout << GREEN << "[-] fix error: stmt num > 150" << RESET << endl;)
        return false;
      }
      ...
      if (res == false) {
        connect_back(m_save);
        connect_back(m_substmt_save);
        debug_log(cout << GREEN << "[-] fix error: fix substmt error" << RESET << endl;)
        return false;
      }
    }
 ...
 * 将 IR 树按照指定类型分割成独立语句
 * @param root IR 树的根节点
 * @param m_save 用于保存被分割节点的映射关系，便于后续恢复
 * @param split_set 需要分割的 IR 类型集合
 * @return 包含所有被分割出来的 IR 节点的向量
 */
vector<IR *> Mutator::split_to_stmt(IR *root, map<IR **, IR *> &m_save,
                                    set<IRTYPE> &split_set) {
  // 存储分割结果的向量
  vector<IR *> res;
  // 使用双端队列进行广度优先搜索，初始化时包含根节点
  deque<IR *> bfs = {root};
  ...
  while (!bfs.empty()) {
    ...
    if (node->left_ && find(split_set.begin(), split_set.end(),
      ...
      // 保存左子节点指针的映射关系，便于后续恢复
      m_save[&node->left_] = node->left_;
      // 将父节点的左子指针设为 NULL，断开连接
      node->left_ = NULL;
    }
    // 检查右子节点类型是否在分割集合中
    if (node->right_ && find(split_set.begin(), split_set.end(),
      ...
      // 保存右子节点指针的映射关系，便于后续恢复
      m_save[&node->right_] = node->right_;
      // 将父节点的右子指针设为 NULL，断开连接
      node->right_ = NULL;
    }
  }
  ...
  while (!bfs.empty()) {
    ...
    if (find(int_types_.begin(), int_types_.end(), node->type_) !=
      ...
      if (get_rand_int(100) > 50)
        node->int_val_ = vector_rand_ele(value_library_);// 从特殊值库中随机选择一个整数值（最大值和最小值）
      else
...
bool Mutator::fill_one(IR *ir) {
  ...
  if (isDefine(ir->data_flag_)) {
    ...
    // 从全局 data_library初始化一个随机的值，比如函数名随机初始化
