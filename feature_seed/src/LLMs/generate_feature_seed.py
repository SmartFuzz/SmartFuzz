...
def seed2_generation():
    for db_name in ['clickhouse','duckdb','virtuoso']:
    # db_name = 'monetdb'
    # db_name = 'clickhouse'
    # db_name = 'duckdb'
...
if __name__ == '__main__':
    for db_name in ['clickhouse','duckdb','virtuoso']:
    # for db_name in ['monetdb']:
        # feature_type='function'
        for feature_type in ['function','operator',"datatype",'keyword']:
            function_json_files = read_FNB_json_files(db_name,feature_type)
            error_file = []
            pbar=tqdm(function_json_files)
            for file in pbar:
                pbar.set_description(f"处理文件 {file}")
                ...
                try:
                    ...
                    # print(f"正在处理文件：{f}")
                        ...
                        output_abs_path = os.path.join(current_path, f"../prepare/FeatureKnowledgeBaseSeed/{db_name}/simpleSeed")
                        # 检测两种输出文件格式是否处理过
                        os.makedirs(output_abs_path, exist_ok=True)
                        # if os.path.exists(os.path.join(output_abs_path, f"{Title}.json")):
                        #     print(f"文件 {file} 已处理，跳过")
                        #     continue
                        sanitize_title = sanitize_filename(normalize_title(f"simple_{feature_type}_{Title}"))
                        if os.path.exists(os.path.join(output_abs_path, f"{sanitize_title}.json")):
                            print(f"文件 {file} 已处理，跳过")
                            continue
        ...
        **deeply combining the  Functions, and Operators**.
        ...
        ## Generation Requirements
        - generate simple example of use the {feature_type}
        - The example must be simple and easy to understand
        - The example must cover the basic usage of the {feature_type}
        - The example must be executable in the {db_name} database
        - generate 5 SQL statements in total
        - with different statement type(create,insert,update,select...)
        # Output Requirements
        ...
        """
                        ...
                        sql_array = parse_llm_json(chat_result["content"])
                        sql_json={}
                        sql_json["feature_type"] = feature_type
                        sql_json["feature_name"] = Title
                        sql_json['sql_array'] = sql_array
                        # 导出
                        with open(os.path.join(output_abs_path, f"{sanitize_title}.json"), 'w') as f:
                            json.dump(sql_json, f, indent=4)
