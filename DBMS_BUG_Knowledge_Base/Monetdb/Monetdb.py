...
def extract_sql_from_text(text):
    sql_pattern = re.compile(r"""
            (?:
                # ========== DDL ==========
                (?:CREATE\s+(?:TABLE|VIEW|INDEX|SCHEMA)\b[^;]*?;) |
                (?:ALTER\s+TABLE\b[^;]*?;) |
                (?:DROP\s+(?:TABLE|VIEW|INDEX|SCHEMA)\b[^;]*?;) |

                # ========== DML ==========
                (?:INSERT\s+INTO?\b[^;]*?;) |
                (?:UPDATE\b[^;]*?;) |
                (?:DELETE\s+FROM\b[^;]*?;) |
                (?:SELECT\b[^;]*?(?:;\s*|$)) |   # 注意：SELECT 可能无分号结尾？这里尽量匹配到;

                # ========== 事务控制 ==========
                (?:START\s+TRANSACTION\s*;) |
                (?:BEGIN(?:\s+WORK|\s+TRANSACTION)?\s*;) |
                (?:COMMIT(?:\s+WORK|\s+TRANSACTION)?\s*;) |
                (?:ROLLBACK(?:\s+WORK|\s+TRANSACTION)?\s*;) |
                (?:SAVEPOINT\s+\w+\s*;) |
                ...
                # ========== 元数据查询（MonetDB 使用系统表）=========
                (?:SELECT\s+.*?\s+FROM\s+sys\.\w+[^;]*?;) |
                (?:SELECT\s+.*?\s+FROM\s+information_schema\.\w+[^;]*?;) |

                # ========== EXPLAIN 执行计划 ==========
                (?:EXPLAIN\s+(?:PLAN\s+)?[^;]*?;) |

                # ========== 删除操作（MonetDB 更倾向 DELETE 而非 TRUNCATE）=========
                (?:DELETE\s+FROM\s+\w+[^;]*?;) 
            )
...
def save_bugs_json(bugs, base_dir):
    ...
    for bug in bugs:
        ...
        bug_report = {
            "url": bug["url"],
            "title": bug["title"],
            "vulnerable_database": "monetdb",  # 你可能需要从 body 或 label 中提取
            "date": bug["createdAt"],
...
if __name__ == '__main__':
    monetdb_bugs = search_issues_bug(5)
    to_csv(monetdb_bugs,"monetdb_bugs.csv")
    print(f"共找到 {len(monetdb_bugs)} 个 bug")

    save_bugs_json(monetdb_bugs, current_dir)


    view_keywords = {'view','视图'}
    view_bugs = bug_filter(monetdb_bugs,view_keywords)
    to_csv(view_bugs,"monetdb_bugs_about_view.csv")
